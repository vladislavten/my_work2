...
def timer(cls, func, date_format):
    def wrapped(*args, **kwargs):
        ...
        print(f"Запускается '{cls._name}.{func.name_}'. Дата и время запуска: {datetime.now().strftime(format)}")
        start = time.time()
        result = func(*args, **kwargs)
        end = time.time()
        print(f"Завершение '{cls._name}.{func.name_}', время работы = {round(end - start, 3)} сек.")
        return result
